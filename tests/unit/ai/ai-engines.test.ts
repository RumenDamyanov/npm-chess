...
describe('AI Engines', () => {
  describe('RandomAI', () => {
    ...
    it('should return a legal move', async () => {
      ...
      const isLegal = legalMoves.some(
        (m: Move) => m.from === move.from && m.to === move.to
      );
  ...
  describe('AI Comparison', () => {
    it('Minimax should find better moves than Random', async () => {
      ...
      const legalMoves = game.getLegalMoves();
      expect(legalMoves.some((m: Move) => m.from === randomMove.from && m.to === randomMove.to)).toBe(true);
      expect(legalMoves.some((m: Move) => m.from === minimaxMove.from && m.to === minimaxMove.to)).toBe(true);
    });
  });
